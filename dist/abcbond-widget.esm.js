...
`;var S="https://abcbond-api.damp-hall-9ea0.workers.dev";function C(){try{let o=localStorage.getItem("widget-state");if(o)return JSON.parse(o).user?.token}catch(o){console.error("Failed to get auth token:",o)}return null}async function v(o,t={}){try{let e=C(),n={"Content-Type":"application/json",...t.headers};e&&(n.Authorization=`Bearer ${e}`);let i=await fetch(`${S}${o}`,{...t,headers:n});if(!i.ok)throw i.status===401?(localStorage.removeItem("widget-state"),window.location.hash="#/login",new Error("Unauthorized - redirecting to login")):new Error(`HTTP error! status: ${i.status}`);return await i.json()}catch(e){throw console.error("API call failed:",e),e}}async function w(o,t){try{let e=await v("/auth/login",{method:"POST",body:JSON.stringify({username:o,password:t})});return e&&e.token?{id:e.user.id,username:e.user.username,email:e.user.email,name:e.user.name,token:e.token}:null}catch(e){return console.error("Login failed:",e),null}}async function L(){let o=await v("/investments");return o&&Array.isArray(o.investments)?o.investments:Array.isArray(o)?o:(console.warn("Invalid investments response format:",o),[])}async function k(o){let t=await v(`/investments/${o}`);return t&&t.investment?t.investment:t}function a(o,t={}){let e=document.createElement(o);return Object.keys(t).forEach(n=>{n==="className"?e.className=t[n]:n==="innerHTML"?e.innerHTML=t[n]:n==="style"&&typeof t[n]=="object"?Object.assign(e.style,t[n]):n==="dataset"?Object.keys(t[n]).forEach(i=>{e.dataset[i]=t[n][i]}):e.setAttribute(n,t[n])}),e}var l=class{constructor(t={}){this.props=t,this.investments=[],this.onInvestmentClick=t.onInvestmentClick||(()=>{}),this.onLogout=t.onLogout||(()=>{}),this.state=t.state}async loadInvestments(){try{return this.investments=await L(),this.investments}catch(t){throw console.error("Failed to load investments:",t),t}}render(){let t=a("div",{className:"investment-list-container"}),e=this.renderHeader();t.appendChild(e);let n=a("div",{className:"investment-list-wrapper"});return t.appendChild(n),this.loadInvestments().then(()=>{n.innerHTML="";let i=this.renderInvestmentList();n.appendChild(i)}).catch(i=>{n.innerHTML="";let r=a("div",{className:"widget-error",innerHTML:`Failed to load investments: ${i.message}`});n.appendChild(r)}),t}renderHeader(){let t=a("div",{className:"investment-header"}),e=this.state?this.state.getCurrentUser():null;return t.innerHTML=`
    ...
    `,t.querySelector("#logout-btn").addEventListener("click",()=>{this.onLogout()}),t}renderInvestmentList(){let t=a("div",{className:"investment-list"});return this.investments.forEach(e=>{let n=this.renderInvestmentCard(e);t.appendChild(n)}),t}renderInvestmentCard(t){let e=a("div",{className:"investment-card"});return e.innerHTML=`
